...
This indicates that the only possible easy way to transform this data is by using *-Y^-3*


```python
df2 = df.copy()
df2.LotArea = ((-1) / (df2.LotArea ** 3))
...
```
    ...
    <matplotlib.axes._subplots.AxesSubplot at 0x7f369a9bd7b8>
...
This obviously isn't working so we are going to try other methods.

The first we are going to try is using the natural logarithm, witch gives the following results.
...
```
    ...
    <matplotlib.axes._subplots.AxesSubplot at 0x7f3685bcda58>
...
This isn't completely normalized, but it's most likeley the closes we can get with the given methods.
